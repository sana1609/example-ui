...
# --------------------------------------------------------------------------
# File name : impact_discovery_ui_angular.py
    ...
    def get_files(self):
        try:
            ...
            for file in files:
                ...
                if extension[-1] == 'js' or extension[-1] == 'jsx' or extension[-1] == "ts":
    ...
    def get_api_linked_files(self):
        try:
            ...
            if len(get_files) == 1:
                ...
                angular_files = get_files[0]
                repo_name = get_files[1]
                matched_files = []
                if len(angular_files) == 0:
                    ...
                    for file in angular_files:
                        ...
                        if file_name[-1] != 'serviceWorker.js' and file_name[-1] != 'serviceworker.js':
                            ...
                            if type(contents) is dict:
                                ...
                                ui_urls_put = re.findall("http.put.*?\\)", text)
                                ui_urls_get = re.findall("http.get.*?\\)", text)
                                ui_urls_delete = re.findall("http.delete.*?\\)", text)
                                ui_urls_patch = re.findall("http.patch.*?\\)", text)
                                ui_urls_post = re.findall("http.post.*?\\)", text)
                    ...
                    [print(x) for x in matched_files]
...
@app.route('/impact_discovery/angular', methods=["POST"])
def impact_discovery():
    try:
        ...
        api_linked_files_final = pd.DataFrame(data=api_linked_files, columns=["File_name", "Method", "URL"])
